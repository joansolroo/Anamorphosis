...
cbuffer cbPerObj : register( b1 )
{
	...
	float4x4 tTex <string uiname="Texture Transform"; bool uvspace=false; >;
...
vs2ps VS(VS_IN input)
{
	...
	Out.TexCd.xy= Out.TexCd.xy;
	Out.TexCd.xyzw /= Out.TexCd.w;
...
{
    float4 col = float4(In.TexCd.xy,In.TexCd.w,1) * cAmb;
	col = mul(col, tColor);
	col.a *= Alpha;
    return col;
}
